...
# payload at that URL. The ONIE guest VM (see run-onie-vm.sh) attaches to $TAP.
#
...
#   HTTP_PORT     8080
#   PAYLOAD_DIR   /tmp/payload   (served over HTTP; place the demo installer
#                                 here as $PAYLOAD_DIR/onie-installer)
...
INSTALLER_URL=${INSTALLER_URL:-http://[$V6_SERVER]:8080/onie-installer}
MTU=${MTU:-1400}
...
teardown() {
    ...
    for pf in "$DNSMASQ_PID" "$HTTP_PID"; do
        if [ -f "$pf" ]; then
            ...
            case "$(cat "/proc/${pid:-0}/comm" 2>/dev/null)" in
                dnsmasq|python3*) kill "$pid" 2>/dev/null || true ;;
            esac
            rm -f "$pf"
        fi
    done
...
[ -e "$PAYLOAD_DIR/onie-installer" ] \
    || die "no 'onie-installer' file in $PAYLOAD_DIR: copy build/images/demo-installer-x86_64-kvm_x86_64-r0.bin there first."
...
# Disable IGMP/MLD snooping: with no querier on this private segment
# the bridge would silently drop the IPv6 solicited-node multicast
# used for global-scope ND, and the guest would see 'Connection
# refused' / 'timed out' when talking to $V6_SERVER.
...
# Disable Duplicate Address Detection on the server-side veth BEFORE
# adding the address: DAD marks a fresh address 'tentative' for ~1s
# and any bind() during that window fails with EADDRNOTAVAIL (errno
# 99).  Also pass 'nodad' on the add itself so the flag applies even
# if the sysctl race is lost.  This is a private two-host link, so
# duplicate detection has zero value here.
ip netns exec "$NETNS" sysctl -q -w net.ipv6.conf.$VETH_NS.accept_dad=0
ip netns exec "$NETNS" sysctl -q -w net.ipv6.conf.$VETH_NS.dad_transmits=0
ip netns exec "$NETNS" ip -6 addr add "$V6_SERVER/64" dev "$VETH_NS" nodad
...
log "Now run ./run-onie-vm.sh (attaches to $TAP), then verify-dhcp6-boot.sh."
